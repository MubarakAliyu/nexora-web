          ...
          <>
            ...
            {billedToTenant(ticket) && ticket.invoiceNumber && (
              <div className="rounded-xl border border-primary/30 bg-primary/5 p-4">
                ...
                <p className="mt-1 font-heading text-h2 font-semibold text-foreground">{formatCurrency(ticket.invoiceAmount ?? ticket.cost ?? 0, ticket.currency, { rateAtCreation: ticket.exchangeRateAtCreation })}</p>
    ...
    { key: "createdAt", header: "Submitted", sortable: true, align: "right", render: (t) => formatDate(t.createdAt) },
    {
      ...
      render: (t) => billedToTenant(t) && t.invoiceNumber
        ? (t.paymentStatus === "awaiting_payment"
          ? <Button size="sm" onClick={(e) => { e.stopPropagation(); setPayingCharge(t); }}>Pay {formatCurrency(t.invoiceAmount ?? 0, t.currency, { rateAtCreation: t.exchangeRateAtCreation })}</Button>
          ...
          {openCharges.map((t) => (
            <Card key={t.id} className="flex flex-col items-start justify-between gap-4 border-l-4 border-accent p-5 sm:flex-row sm:items-center">
              <div>
                ...
                <p className="mt-1 font-heading text-h3 font-semibold text-foreground">{formatCurrency(t.invoiceAmount ?? t.cost ?? 0, t.currency, { rateAtCreation: t.exchangeRateAtCreation })}</p>
